...
export class ListUserStoryComponent implements OnInit {
  ...
  lostfocusTest(event) {
    ...
    if (this.testlink.indexOf("http") < 0 || this.testlink.length == 0) {
      this.elementTest.innerText = this.elementTest.parentElement.children[7].innerText;
    }
    else {
      // tslint:disable-next-line:max-line-length
  ...
  lostfocusDoc(event) {
    ...
    if (this.doclink.indexOf("http") < 0 || this.doclink.length == 0) {
      this.elementDoc.innerText = this.elementDoc.parentElement.children[9].innerText;
    }
    else {
      // tslint:disable-next-line:max-line-length
